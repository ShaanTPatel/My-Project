...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 23 16:21:13 2023
...
# Load the CSV file
file_path = "/users/shaanpatel/desktop/Data 2/60km/MeanPrecipRCP85Global.csv"
...
models = annual_data.columns[0:30]  # Exclude the 'Year' column
...
plt.title('HadGEM3 Models - Mean Precipitation Rate', fontsize=16)
...
plt.xlabel('Year')
plt.ylabel('Mean Precipitation Rate (mm/day)')
plt.title('Annual Mean Precipitation Rate from 1900-2099 using HadGEM3-GC3.05')

# Move the legend outside the plot area
plt.legend(fontsize=7, bbox_to_anchor=(1.05, 1), loc='upper left')
plt.xticks(np.arange(1900, 2100, 10), rotation='vertical', fontsize=5)
plt.show()
...
hadg3_summary_file_path = file_path.replace('.csv', '_hadgem3_summary_table.csv')
hadg3_summary_table.to_csv(hadg3_summary_file_path, index=False)

# Print the HadGEM3 summary table
print("HadGEM3 Models Summary Table:")
print(hadg3_summary_table)

# Plot other models together on a separate plot
plt.figure(figsize=(10, 6))
plt.title('Other Models - Mean Precipitation Rate', fontsize=16)
...
plt.xlabel('Year')
plt.ylabel('Mean Precipitation Rate (mm/day)')
plt.title('Annual Mean Precipitation Rate from 1900-2099 using the CMIP5 multi-model ensemble')
...
other_summary_file_path = file_path.replace('.csv', '_other_models_summary_table.csv')
other_summary_table.to_csv(other_summary_file_path, index=False)

# Print the other models summary table
print("Other Models Summary Table:")
